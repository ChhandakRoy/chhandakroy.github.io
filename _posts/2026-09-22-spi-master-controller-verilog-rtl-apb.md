...
<figure>
  <img src="{{ '/assets/images/spi-master-controller/apb_fsm.png' | relative_url }}" alt="APB state machine" style="max-width:100%;height:auto;">
...
<figure>
  <img src="{{ '/assets/images/spi-master-controller/reg_map.png' | relative_url }}" alt="SPI controller register map" style="max-width:100%;height:auto;">
  <figcaption><strong>Figure 6:</strong> Register map of the APB-visible SPI controller.</figcaption>
</figure>
...
```mermaid
flowchart LR
    A[APB Write or Read] --> B[Control and Data Registers]
    B --> C[SPI Control and Datapath]
    C --> D[Status]
    D --> A
```

The processor does not have to manually control individual SCLK edges. It configures the controller through registers, and the hardware performs the serial transfer.
...
The resulting divider controls the timing of SCLK and the event pulses used by the shift register.

### Why use a programmable divider?

The APB/system clock is normally faster than the desired SPI serial clock. A programmable divider lets software select the SPI transfer rate without changing the system clock.

### SCLK generation

The baud generator:

- Holds SCLK at the configured `CPOL` idle level when no transfer is active.
- Loads the active baud division value while the interface is idle.
- Counts `PCLK` cycles during an active transfer.
- Toggles SCLK when the half-period count is reached.
- Produces timing-event pulses associated with the SPI transmit/receive operations.
